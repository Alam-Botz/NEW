		...
		try {
            ...
            Ramdani.sendMessage(anu.jid, 'Halo! Terima Kasih sudah Mengundangku, Jika ingin Menggunakan Bot Ketik ${prefix}menu', 'conversation')
             ...
             if (!welkom.includes(anu.jid)) return
	            ...
	            buff = fs.readFileSync(`media/picture/welcome.jpg`) 
                ...
                buttonsMessage = { contentText: `${teks}`, footerText: 'welcome', imageMessage: imageMsg, buttons: buttons, headerType: 4 }
            ...
            if (!welkom.includes(anu.jid)) return
                ...
                memeg = mdata.participants.length
                out = `Byee ${anu_user}`
                buff = fs.readFileSync(`media/picture/out.jpg`) 
                buttons = [{buttonId: `.`,buttonText:{displayText: 'BYEE'},type:1}]
                imageMsg = (await Ramdani.prepareMessageMedia((buff), 'imageMessage', {thumbnail: buff})).imageMessage
                buttonsMessage = { contentText: `${out}`, footerText: 'Byee', imageMessage: imageMsg, buttons: buttons, headerType: 4 }
                prep = await Ramdani.prepareMessageFromContent(mdata.id,{buttonsMessage},{})
                Ramdani.relayWAMessage(prep)
            }
		} catch (e) {
